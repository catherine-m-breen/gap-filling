...
class ASODataset(Dataset):
    def __init__(self, data, labels, crop_size=(128, 128), augment=False):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            ...
            combined = np.concatenate([data_item, label_item], axis=0)  # Stack along channel
            combined = random_crop(combined[None, :, :, :], self.crop_size)[0]
            data_item = combined[:-1, :, :]  # All channels except last
            label_item = combined[-1:, :, :]  # Last channel
...
def load_zarr_patches(zarr_dir, split_dict, flight_to_basin_dict):
    """
    Load all zarr patch files and organize by train/val/test split.
    ...
    print(f"Loading {len(zarr_files)} zarr patch files...")
    
    for zarr_path in zarr_files:
        # Get basin and split from filename
        # Assuming filename format: {flight_id}_r{row}_c{col}.zarr
        parts = zarr_path.stem.split('_r')
        flight_id = parts[0]
        ...
        X = np.array(z['X'], dtype=np.float32)  # (17, H, W)
        Y = np.array(z['Y'], dtype=np.float32)  # (1, H, W)
    ...
    print(f"Loaded: {len(train_x)} train, {len(val_x)} val, {len(test_x)} test patches")
...
def main():
    # Config
    zarr_dir = "/discover/nobackup/cmbreen/gap-filling-data/zarr_patches_128x128"
    ...
    train_x, train_y, val_x, val_y, test_x, test_y = load_zarr_patches(
    ...
    print(f"Training samples: {len(train_x_norm)} original + {len(augmented_x)} augmented = {len(combined_train_x)} total")
    
    # Create datasets
    train_dataset = ASODataset(combined_train_x, combined_train_y, augment=False)
    val_dataset = ASODataset(val_x_norm, val_y_norm, augment=False)
    ...
    input_channels = 17  # After one-hot encoding
