...
def random_flip(data, labels, masks):
    ...
    if flip_w:
        ...
        masks = np.flip(masks, axis=1).copy()  # this is the Y mask
    if flip_h:
        data = np.flip(data, axis=3).copy()
        labels = np.flip(labels, axis=2).copy()
        masks = np.flip(masks, axis=2).copy() # this is the Y mask
    
    return data, labels, masks


def add_gaussian_noise(image, mean=0, sigma=25, noise_channels = [0]):
    """
    Add Gaussian noise to specific channels only.
    
    Args:
        image: Input image (1, C, H, W)
        mean: Mean of Gaussian noise
        sigma: Standard deviation of noise
        noise_channels: List of channel indices to add noise to (e.g., [0, 1, 2, 3, 4])
    """
    _, ch, row, col = image.shape
    noisy = image.copy()  # copy to preserve all data, make sure it is not np.zeros
    # Add noise to each specified channel
...
def load_full_zarr_files(zarr_dir, split_dict, flight_to_basin_dict, skip_tb_channels=True):
    ...
    for zarr_path in zarr_files:
        ...
        # ========================================
        extreme_mask = Y > 10.0 ## there are a couple of places but it isn't too bad 
...
def main():
    ...
    checkpoint_dir = "./checkpoints_elevPM_NDSI_CC_1e-6_ps256_W3_smoothL1loss"
