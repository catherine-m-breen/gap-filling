...
for aso_flight in asopath:
   ...
   # unforested = (canopy <= 40).astype("float32")
    ...
    # one hot encoded snow class 
    snow = predictors[0]  # Shape: (H, W)
    snow_classes = [1, 2, 3, 4, 5, 6, 7]
    snow_one_hot = []
    for class_id in snow_classes:
        snow_class_mask = (snow == class_id).astype("float32")
        snow_one_hot.append(snow_class_mask[None, :, :])  # Add channel dimension

    # Concatenate one-hot encoded snow classes to X
    snow_one_hot = np.concatenate(snow_one_hot, axis=0)  # Shape: (6, H, W)
    X[:] = np.concatenate([X, snow_one_hot], axis=0)
